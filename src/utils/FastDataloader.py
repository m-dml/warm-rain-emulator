...
class DataModule(pl.LightningDataModule):
    ...
    def get_tendency(self):
        ...
        for i in range(self.arr.shape[-2]):
            ...
            if self.ic == "small":
                ...
                for k in range(0, tot_col, 4):
                    new_output_tend[:, k : k + 4, :] = c_arr[
                        j : (l - self.step_size + j), :, :
                    ]
                    j += 1
                self.output_tend = new_output_tend.transpose(2, 0, 1).reshape(
                    -1, tot_col
                )
                # self.output_tend= c_arr.transpose(0,2,1).reshape(-1,4)
                self.output_tend_all.append(self.output_tend)
        ...
        if self.normalize == "Rel":
            ...
            self.updates, self.updates_mean, self.updates_std = self.norm(
                np.asarray(self.output_tend_all)
            )
        else:
        ...
        for i in range(self.arr.shape[-2]):
            ...
            if self.ic == "small":
                if (self.arr[0, -4, i, 0] > 0.0008) and (self.arr[0, -2, i, 0] < 1.5):
                    ...
                    tau = self.new_arr[: l - self.step_size, 3, i, :] / (
                        self.new_arr[: l - self.add_argparse_argsstep_size, 3, i, :]
                        + self.new_arr[: l - self.step_size, 1, i, :]
                    )
                    xc = self.new_arr[: l - self.step_size, 1, i, :] / (
                        self.new_arr[: l - self.step_size, 2, i, :]
                    )
                    ...
                    inputs = np.concatenate(
                        (sim_data_1, tau.reshape(-1, 1), xc.reshape(-1, 1), sim_data_2),
                        axis=1,
                    )
                ...
                tau = self.new_arr[: l - self.step_size, 3, i, :] / (
                    self.new_arr[: l - self.step_size, 3, i, :]
                    + self.new_arr[: l - self.step_size, 1, i, :]
                )
                xc = self.new_arr[: l - self.step_size, 1, i, :] / (
                    self.new_arr[: l - self.step_size, 2, i, :]
    ...
    def test_train(self):
        ...
        self.dataset = my_dataset(
            self.inputs[:, :], self.meta[:, :], self.updates[:, :], self.outputs[:, :]
        )
