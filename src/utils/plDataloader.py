...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self):
        ...
        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/inputs_mean.npy",
            self.inputs_mean,
        )
        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/updates_mean.npy",
            self.updates_mean,
        )
        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/outputs_mean.npy",
            self.outputs_mean,
        )

        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/inputs_std.npy", self.inputs_std
        )
        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/updates_std.npy",
            self.updates_std,
        )
        np.save(
            "/gpfs/work/sharmas/mc-snow-data/rel_update/outputs_std.npy",
            self.outputs_std,
        )
        ...
        for i in range(self.sim_num):
            ...
            for j in range(819):
                ...
                for t in range(1, len(sim_data)):
                    output_sim.append(
                        (sim_data[t, 1:5] - sim_data[t - 1, 1:5])
                        / (20 * (sim_data[t - 1, 1:5]))
                    )
