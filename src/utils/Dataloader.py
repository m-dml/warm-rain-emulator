...
def create_dataset_updatefunc(d, time):
    ...
    dataset = np.concatenate(
        (np.asarray(timestep).reshape(-1, 1), np.asarray(inputs), np.asarray(outputs)),
        axis=1,
    )
...
def create_dataset_PR_nets(d, time, input_num=5):
    ...
    dataset = np.concatenate(
        (
            np.asarray(timestep).reshape(-1, 1),
            np.asarray(inputs),
            np.asarray(outputs),
            np.asarray(mom),
            np.asarray(meta),
...
def norm_data(ds, norm_start=0, norm_end=None):
    if norm_end == None:
        norm_end = ds.shape[-1]
    means, sds = np.full(norm_end - norm_start, np.nan), np.full(
        norm_end - norm_start, np.nan
    )
    n = 0
