...
class simulation_forecast:
    ...
    def test(self):
        ...
        self.real_data = self.test_sims[:, 1:, self.sim_number]
        self.model_params = self.sim_data[-4:-1]
        self.lo = self.sim_data[0] + self.sim_data[2]
        self.model_params[0] = self.lo
        self.xx = []
        self.xy = []
        # #self.err=np.zeros((np.ma.compress_rows(self.test_sims[:, :, self.sim_number]),
        #                                        self.step_size,4))
        for i in range(
            0, np.ma.compress_rows(self.test_sims[:, :, self.sim_number]).shape[0],self.step_size
        ):
            self.xx_sub = np.zeros((self.step_size,4))
            self.xy_sub = np.zeros((self.step_size,4))
            self.sim_data = self.test_sims[i, 1:, self.sim_number]
            #Take stuff from real sim here
            self.create_input()
            self.model.eval()
            predictions_updates = self.model.test_step(torch.from_numpy(self.inputs))
            self.moment_calc(predictions_updates)

            self.xx_sub[0,:] = self.preds.reshape(1,4)
            self.xy_sub[0,:] = self.real_data[i,:4]
            #mse_sqr_diff= (self.preds.reshape(1,4) - self.real_data[i,:4])**2
            #print( mse_sqr_diff.shape)
            #self.err[0,:] = self.err[0,:]+ mse_sqr_diff.reshape(-1,4)
            self.sim_data = self.preds.reshape(
            -1,
        )
            for j in range(1,self.step_size):
                if i+j<np.ma.compress_rows(self.test_sims[:, :, self.sim_number]).shape[0]:
                    self.create_input()
                    predictions_updates = self.model.test_step(torch.from_numpy(self.inputs))
                    self.moment_calc(predictions_updates)
                    self.xx_sub[j,:] = self.preds.reshape(1,4)
                    self.xy_sub[j,:] = self.real_data[i+j,:4]
                    #mse_sqr_diff= (self.preds.reshape(1,4) - self.real_data[i+j,:4])**2
                    #self.err[j,:] = self.err[j,:] + mse_sqr_diff.reshape(-1,4)
                    self.sim_data = self.preds.reshape(
                -1,
        )
            self.xx.append(self.xx_sub)
            self.xy.append(self.xy_sub)
    ...
    def check_preds(self):
        ...
        #self.preds[:, 0] = self.lo - self.preds[:, 2]
